...
sSymbol_t cfunc = 0;		/* current function */
char *fname = 0;		/* current function name */
sSymbol_t retv;			/* return value location for structs */

typedef sSymbol_t (*fDcl_t)(int, char *, sType_t, sCoordinate_t *);

static sList_t autos;		/* auto locals for current block */
...
static sList_t registers;		/* register locals for current block */

static void checklab(sSymbol_t, void *);
static void checkparam(sSymbol_t, void *);
static void checkref(sSymbol_t, void *);
static sSymbol_t dclglobal(int, char *, sType_t, sCoordinate_t *);
static sSymbol_t dcllocal(int, char *, sType_t, sCoordinate_t *);
static sSymbol_t dclparam(int, char *, sType_t, sCoordinate_t *);
static sType_t dclr(sType_t, char **, int);
static sType_t dclr1(char **, int);
static void decl(fDcl_t, int);
static void doglobal(sSymbol_t, void *);
static void doextern(sSymbol_t, void *);
static sType_t enumdecl(void);
static void fields(sType_t);
static void funcdecl(int, char *, sType_t, sCoordinate_t);
static sType_t newstyle(sType_t, sSymbol_t[], sSymbol_t[]);
static void oldparam(sSymbol_t, void *);
static sType_t oldstyle(char *, sType_t, sSymbol_t[], sSymbol_t[]);
static sType_t *parameters(int);
static sType_t structdcl(int);
static sType_t tnode(int, sType_t);
static sType_t type(int, int *);

/* checklab - check for undefined labels; called at ends of functions */
static void checklab(sSymbol_t p, void *cl)
{
	if (!p->defined)
...
static void checkref(sSymbol_t p, void * cl)
{
	sSymbol_t q;
...
void compound(int loop, sSwtch_t swp, int lev){
	sSymbol_t p;
	Code cp;
	int i, j, nregs;
	...
	if (level == LOCAL && events.entry)
		apply(events.entry, (void *)cfunc, (void *)0);
	expect('{');
	while (kind[t] == CHAR || kind[t] == STATIC
          || t == ID && tsym && tsym->sclass == TYPEDEF && (level < LOCAL || getchr() != ':'))
		decl(dcllocal, 0);
	nregs = length(registers);
	cp->u.block.locals = (sSymbol_t *)ltoa(registers, (void * *)talloc((nregs + length(autos) + 1)*sizeof(sSymbol_t)));
	ltoa(autos, (void * *)&cp->u.block.locals[nregs]);
	while (kind[t] == IF || kind[t] == ID)
		statement(loop, swp, lev);
	walk(0, 0, 0);
	foreach(identifiers, level, checkref, (void *)0);
...
static sSymbol_t dclglobal(sclass, id, ty, pos) char *id; sType_t ty; sCoordinate_t *pos; {
	sSymbol_t p, q;
...
static sSymbol_t dcllocal(sclass, id, ty, pos) char *id; sType_t ty; sCoordinate_t *pos; {
	sSymbol_t p, q;
...
static sSymbol_t dclparam(sclass, id, ty, pos) char *id; sType_t ty; sCoordinate_t *pos; {
	sSymbol_t p;
...
/* dclr - declarator */
static sType_t dclr(basety, id, lev) sType_t basety; char **id; {
	sType_t ty;
...
static sType_t dclr1(id, lev) char **id; {
	sType_t ty = 0;
	...
	switch (t) {
      ...
      case '(':
         ...
         if (kind[t] == CHAR || t == ID && tsym && tsym->sclass == TYPEDEF) {
            ...
            { foreach(identifiers, level, checkparam, (void *)0); exitscope(); }
         } else {
            ty = dclr1(id, lev);
            expect(')');
            if (ty == 0 && id == 0)
...
/* decl - type [ dclr ( , dclr )* ] ; */
static void decl(fDcl_t dcl, int eflag)
{
	int sclass;
	char *id = 0;
	sType_t ty, ty1;
...
static void doextern(p, cl) sSymbol_t p; void * cl; {
	sSymbol_t q;
...
static void doglobal(p, cl) sSymbol_t p; void * cl; {
...
static sType_t enumdecl() {
	...
	if (t == '{') {
		...
		ty->u.sym->u.idlist = (sSymbol_t *)ltoa(idlist, (void * *)alloc((length(idlist) + 1)*sizeof(sSymbol_t)));
		ty->u.sym->defined = 1;
...
static void fields(ty) sType_t ty; {
...
void finalize() {
	if (xref) {
		setuses(identifiers);
		foreach(types, level, fielduses, (void *)0);
		setuses(types);
	}
...
static void fillcallees(sSymbol_t, void *);

static void fillcallees(p, cl) sSymbol_t p; void * cl; {
	*(sList_t *)cl = append(p, *(sList_t *)cl);
}

/* funcdecl - ... ( ... ) decl* compound */
static void funcdecl(sclass, id, ty, pt) char *id; sType_t ty; sCoordinate_t pt; {
	int i, n;
	Code rp;
	sSymbol_t *callee, *caller, p;
	sList_t callees = 0;
	...
	callees = 0;
	foreach(identifiers, PARAM, fillcallees, (void *)&callees);
	n = length(callees);
	if (Aflag >= 2 && n > 31)
		warning("more than 31 parameters in function `%s'\n", id);
	caller = (sSymbol_t *)ltoa(callees, (void * *)0);
	callee = (sSymbol_t *)talloc((n + 1)*sizeof *caller);
	for (i = 0; p = caller[i]; i++) {
		callee[--n] = p;
		caller[i] = (sSymbol_t)talloc(sizeof *caller[i]);
	...
	expect('}');
	setuses(labels[0]);
	foreach(labels[0], LABELS, checklab, (void *)0);
	if (events.exit)
		apply(events.exit, (void *)cfunc, (void *)0);
	exitscope();
	retv = 0;
	tfree();
	cfunc = 0;
...
static sType_t newstyle(sType_t ty, sSymbol_t caller[], sSymbol_t callee[])
{
	int i;
...
static void oldparam(sSymbol_t p, void * cl)
{
	sSymbol_t *callee = (sSymbol_t *)cl;
	int i;
...
static sType_t oldstyle(char *name, sType_t ty, sSymbol_t caller[], sSymbol_t  callee[])
{
	int i;
	sSymbol_t p;
	...
	while (kind[t] == CHAR || kind[t] == STATIC
          || t == ID && tsym && tsym->sclass == TYPEDEF)
		decl(dclparam, 1);
      foreach(identifiers, PARAM, oldparam, (void *)callee);
      for (i = 0; p = callee[i]; i++) {
         if (p->type == 0) {
            p->type = inttype;
            p->defined = 1;
...
static sType_t *parameters(lev) {
	...
	if (kind[t] == CHAR || kind[t] == STATIC
       || t == ID && tsym && tsym->sclass == TYPEDEF) {
		...
		proto = (sType_t *)ltoa(list, (void * *)alloc((n + 1)*sizeof (sType_t)));
	} else if (t == ID) {
		for (;;) {
			if (t != ID) {
			...
			dclparam(0, token, (sType_t)0, &src);
			t = gettok();
			if (t != ',')
				break;
			t = gettok();
...
static void checkparam(sSymbol_t p, void * cl)
{
	if (p->type == 0)
...
static sType_t tnode(int op, sType_t type)
{
	sType_t ty = (sType_t) talloc(sizeof *ty);
...
static sType_t type(int lev, int *sclass) 
{
